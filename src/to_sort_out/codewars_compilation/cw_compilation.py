...
# https://code.tutsplus.com/tutorials/beginning-test-driven-development-in
# -python--net-30137
...
# https://www.codewars.com/kata/do-you-know-how-to-make-query-string/train
# /python
...
def to_query_string(data):
    ...
    for key, value in sorted(
            data.items()):  # you can sort a dictionary by its keys
        if not isinstance(value, (list, tuple, set)):
            value = [
                value]  # clever to transform non-sequence type to a single
            # element list
        for val in value:
            result.append("{}={}".format(key,
                                         val))  # wow, you can use format
            # instead of .join()!!
    return "&".join(result)
...
def bowling_pins(arr):
    ...
    bowling_pins_graphic_dictionary = {1: 27, 3: 20, 2: 18, 4: 9, 5: 11, 6: 13,
                                       7: 0, 8: 2, 9: 4, 10: 6}
    for elem in arr:
...
def bowling_pins(empty):
    ...
    return ''.join(
        [replace[x] if order[x] in empty else raw[x] for x in range(10)])
...
def is_valid_IP(strng):
    ...
    for i, octet in enumerate(IP_octet_list):
        try:
            ...
            if is_octet_between_0_and_255 and is_length_between_1_and_3 and \
                    is_octet_starts_with_non_0:
...
def main():
    ...
    test_is_valid_IP = {
        test_IP_string: True, test_IP_string2: False, test_IP_string3:
            False, test_IP_string4: False, test_IP_string5: False,
        test_IP_string6: False,
...
def is_valid_IP(strng):
    return bool(
        re.match(r'^((\d{1,2}|1\d{2}|2[0-4]\d|25[0-5])(\.(?!$)|$)){4}(?=$)',
                 strng))
